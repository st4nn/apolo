<?php
   ...
   $sql = "SELECT
         ...
         FROM
            OT
            LEFT JOIN confMunicipios ON OT.idMunicipio = confMunicipios.id
            LEFT JOIN confZonas ON OT.idZona = confZonas.id
            LEFT JOIN confEstados ON OT.idEstado = confEstados.id
            INNER JOIN Levantamiento ON Levantamiento.idProyecto = OT.Prefijo
         WHERE
            OT.idEstado <> 8 
            $where
           GROUP BY
